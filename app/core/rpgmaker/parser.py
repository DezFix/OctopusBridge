...
Извлекает текст только из известных текстоносных полей и команд событий,
чтобы не задеть служебные строки (имена файлов, цвета и т.п.).
"""
...
CMD_CHOICES = 102       # массив вариантов выбора
CMD_CHOICE_BRANCH = 402 # вариант ветки
CMD_COMMENT = 108       # комментарий (1-я строка)
CMD_COMMENT_CONT = 408  # комментарий (продолжение)
CMD_SHOW_TEXT_HDR = 101 # заголовок диалога: параметр 4 = имя говорящего (MZ)
CMD_CHANGE_NAME = 320   # смена имени актёра
CMD_CHANGE_NICK = 324   # смена псевдонима
CMD_PLUGIN = 357        # плагин-команда MZ
CMD_PLUGIN_CONT = 657   # многострочное продолжение плагин-команды
CMD_PLUGIN_MV = 356     # плагин-команда MV (одна строка)

# Поля БД: файл -> список полей верхнего уровня у каждого объекта массива
DB_FIELDS = {
...
def find_data_dir(game_dir: str) -> str:
    """Где лежат данные: 'data' (MZ/MV-проект) или 'www/data' (деплой MV)."""
...
class _Extractor:
    ...
    def event_list(self, file: str, base: str, cmd_list: list, context: str):
        for i, cmd in enumerate(cmd_list):
            ...
            if code in (CMD_DIALOG, CMD_SCROLL, CMD_COMMENT, CMD_COMMENT_CONT):
                if params:
                    kind = "comment" if code in (CMD_COMMENT, CMD_COMMENT_CONT) else "dialog"
                ...
                for j, label in enumerate(params[0]):
                    self.add(file, f"{p}[0][{j}]", f"{context} / choice", label)
                ...
                self.add(file, f"{p}[4]", f"{context} / speaker name", params[4])
            elif code in (CMD_CHANGE_NAME, CMD_CHANGE_NICK) and len(params) > 1:
                self.add(file, f"{p}[1]", f"{context} / rename", params[1])
            elif code == CMD_PLUGIN and len(params) > 3 and isinstance(params[3], dict):
                for k, v in params[3].items():
                    self.add(file, f"{p}[3].{k}", f"{context} / plugin", v)
            elif code == CMD_PLUGIN_CONT:
                for j, v in enumerate(params):
                    self.add(file, f"{p}[{j}]", f"{context} / plugin", v)
            elif code == CMD_PLUGIN_MV and params:
                self.add(file, f"{p}[0]", f"{context} / plugin (MV)", params[0])
    ...
    def db_file(self, file: str, data: list, fields: list[str]):
        for idx, obj in enumerate(data):
            ...
            for field in fields:
                if field in obj:
                    self.add(file, f"[{idx}].{field}", f"{file[:-5]} '{name}'", obj[field])
    ...
    def map_file(self, file: str, data: dict):
        ...
        for ei, ev in enumerate(data.get("events") or []):
            ...
            for pi, page in enumerate(ev.get("pages") or []):
                ctx = f"{file[:-5]} event '{ev.get('name', ei)}' p.{pi + 1}"
                self.event_list(file, f"events[{ei}].pages[{pi}].list",
    ...
    def common_events(self, file: str, data: list):
        for idx, ev in enumerate(data):
            if not isinstance(ev, dict):
                continue
            self.add(file, f"[{idx}].name", f"common event #{idx}", ev.get("name", ""))
            self.event_list(file, f"[{idx}].list", ev.get("list") or [],
    ...
    def troops(self, file: str, data: list):
        for idx, tr in enumerate(data):
            if not isinstance(tr, dict):
                continue
            self.add(file, f"[{idx}].name", f"enemy group #{idx}", tr.get("name", ""))
            for pi, page in enumerate(tr.get("pages") or []):
                self.event_list(file, f"[{idx}].pages[{pi}].list",
                                page.get("list") or [],
                                f"battle '{tr.get('name', idx)}' p.{pi + 1}")

    def system(self, file: str, data: dict):
        self.add(file, "gameTitle", "game title", data.get("gameTitle", ""))
        self.add(file, "currencyUnit", "currency", data.get("currencyUnit", ""))
        for fld in SYSTEM_LIST_FIELDS:
            for j, v in enumerate(data.get(fld) or []):
                self.add(file, f"{fld}[{j}]", f"term {fld}", v)
        # индекс 0 в этих массивах всегда пустая строка (служебный) —
        # add() и так отфильтрует её по .strip(), поэтому просто идём по всем
        ...
        for fld in TERMS_LIST_FIELDS:
            for j, v in enumerate(terms.get(fld) or []):
                self.add(file, f"terms.{fld}[{j}]", f"term {fld}", v)
        for k, v in (terms.get("messages") or {}).items():
            self.add(file, f"terms.messages.{k}", "system message", v)

    def map_infos(self, file: str, data: list):
        for idx, obj in enumerate(data):
            if isinstance(obj, dict) and obj.get("name"):
                self.add(file, f"[{idx}].name", "map name (list)", obj["name"])
...
def extract(game_dir: str, data_dir: str | None = None,
    ...
    (битый JSON/кодировка) — чтобы такие файлы не пропадали молча.
    """
    if data_dir is None:
...
def apply(game_dir: str, entries: list[TranslationEntry],
    ...
    Перезаписывает только записи с непустым переводом и статусом != 'skip'.
    Перед первой записью каждого файла делает бэкап в backup_root.
    on_skip(entry, reason) вызывается для записей, которые не удалось
    применить (не найден путь / текущее значение — не строка).
    """
    if data_dir is None:
        data_dir = find_data_dir(game_dir)
        ...
        for e in items:
            try:
                ...
                if on_skip:
                    ...
                    print(f"[parser] {rel}: path {e.json_path} not found ({exc})")
                continue
            if isinstance(current, str):
                try:
                    ...
                    if on_skip:
                        ...
                        print(f"[parser] {rel}: cannot write {e.json_path} ({exc})")
            elif on_skip:
