...
**Под капотом:** старые переводы автоматически восстанавливаются при повторном извлечении, есть импорт/экспорт CSV, а все движки используют единый кэш (`octopus_cache.json`) в папке игры — старые кэши переносятся автоматически.
