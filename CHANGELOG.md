...
### Added
- **RPG Maker: отчёт о пропусках после Apply** — движок считает `skipped_total`/`skipped_by` (ресурсы, ключи, сдвиги, битые пути) из `on_skip`-причин file-patch; диалог показывает строку «Пропущено: N (...)» (`tr_apply_skipped`, RU/EN).

### Fixed
- **Ren'Py: C0-мусор в переводах режется + самопроверка записи** — `_escape` вырезает управляющие символы (кроме `\n`/`\t`), каждый записанный `ob_*.rpy` проверяется `_written_file_ok`, битый удаляется сразу, а не убивает старт игры.
- **Twine: структурный тест JS-пейлоадов** — баланс скобок + запрет `=>`/backticks/`const`/`let` для обоих скриптов (браузера для прогона JS в CI нет, это ближайшая страховка).

## [7.12] — 2026-09-13

### Fixed
- **RPG Maker: покрытие на второй крысе (Очикано Another, 104k CJK-строк)** — замер каждой группы непереведённого: живой текст закрыт, код/данные/файлы доказательно пропущены.
- **RPG Maker: значения `KEY = value` в 657 — уже покрыты (проверено 3273/3273)**, данные (`変数 = 102`, `Scene_point`, координаты) пропускаются; пропуск `params[2]` в 357 документирован квитанцией из кода игры (`callCommand(this, pluginName, params[1], params[3])` — индекс игнорируется движком, для плагинов это ключи dispatch).
- **RPG Maker: заголовок окна переводится** — извлечение `package.json window.title` + `index.html <title>`, безопасная запись (только поле title, валидация, бэкап; чужой заголовок не затирается) + установка в рантайме (`document.title`, `nw.Window`). Раньше заголовок оставался японским всегда.
- **RPG Maker: кастомные данные разработчика** — каталог `dataEx/` (TRP skit DB и подобные) сканируется generic-путём; неизвестные JSON в `data/` вынесены в `_extract_unknown_json`.
- **RPG Maker: `parser.apply` не затирает чужое** — `_apply_event_slot` распространён и на `.rpgmvm`-ветку (раньше там была слепая перезапись).
- **RPG Maker: verify читает package.json** — битый манифест = игра не стартует вообще.

## [7.11] — 2026-09-13

### Fixed
- **RPG Maker: покрытие перевода на лабораторной игре (tropical-chase)** — замер по всем CJK-строкам `data/`: непереведённый живой текст найден и закрыт, мёртвые/ядовитые строки вычищены (7264 точные записи вместо 11220 с мусором).
- **RPG Maker: значения `KEY = value` в continuation 657** — сотни строк попапов (LL_InfoPopupWIndow), текстов картинок (TextPicture) и ID (Torigoya, ARTM, SceneCustomMenu) лежали в одном формате. Теперь `_split_kv_line`/`_splice_kv`: извлекается только value-часть, ключ цел всегда; в рантайме KV-подмена в `obWalkCmd` (ES5), в `parser.apply` — parity через `_apply_event_slot`. Числа/идентификаторы/сцены (`変数 = 102`, `Scene_point`) по-прежнему пропускаются.
- **RPG Maker: свои плагины не извлекаются** — словарь `ob_runtime.js` (тысячи уже переведённых строк) попадал обратно в выгрузку как «оригиналы» (двойной перевод + мусор в таблице): `ob_runtime`/`octopus_ob` пропущены в `extract_plugins`.
- **RPG Maker: вложенные структуры аргументов 357** — `_extract_plugin_arg` рекурсивно обходит dict/list (с теми же code/res-гардами).
- **RPG Maker: `parser.apply` не затирает чужое** — `_apply_event_slot` пишет слот, только если файл не изменился с извлечения (раньше — слепая перезапись); 657-слоты идут через KV-со reconstruction.
- Проверено по коду самой игры (`rmmz_objects.js`: `callCommand(this, pluginName, params[1], params[3])`): индекс 2 команд 357 игнорируется движком — это мёртвые ключи диспетчера, их пропуск документирован и осознан. 657-слоты в `parser.apply` идут через KV-реконструкцию.

## [7.10] — 2026-09-13

### Fixed
- **Ren'Py: RCE через враждебный .rpyc/.rpa закрыт** — распаковка шла сырым `pickle.loads`/`find_class` без ограничений: файл игры мог выполнить код через `__reduce__` (`os.system`, `eval`). Теперь whitelist: настоящие только renpy-моки, `copyreg.__newobj__`/`_reconstructor` и контейнеры/скаляры builtins; чужие модули уходят в терпимый mock (форма сохраняется, код не выполняется). Индекс `.rpa` переведён на строгий `_SafeIndexUnpickler` (только dict/list/tuple/str/int/bytes) + проверка формы записей.
- **Ren'Py: перевод с добавленными [...]/{...} отклоняется** — `_is_interp_safe` теперь сверяет и количество скобочных групп: несбалансированный тег, привнесённый переводчиком, раньше ронял показ реплики исключением.
- **Ren'Py: одноимённые .rpa в разных подпапках** — кэш архивов и `rpa://`-ссылки переведены на полный путь (был basename): файлы больше не читаются из чужого архива.
- **Twine: возвращена браузерная половина live-перевода** — Python слал `tr_dict`/`tr_set` в пустоту (страница их не обрабатывала). Новый `TR_SCRIPT` (ES5, оба режима инжекции): словарь проекта на текстовых узлах, живые батчи `tr_request`/`tr_result` (40 строк/1800 знаков, один в полёте), `MutationObserver` + `:passagedisplay`, откат оригиналов при выключении (только где лежит наш перевод, чужое не затирается, мёртвые узлы чистятся).
- **Twine: тихая потеря правок сейвов** — `_set_nested` создавал контейнеры без привязки к родителю (запись исчезала молча); теперь привязка сразу + игнор бессмысленных путей.
- **Twine: path traversal в имени слота** — id слота от игры идёт в имя `.save`-файла; `../` теперь чистится, запись за пределы папки невозможна.
- **Twine: мелочи моста** — `flatten_variables({})` больше не даёт ключ `""`; `setVar` создаёт массив под числовой сегмент пути; `for..in` по переменным с `hasOwnProperty`; `.html` с query-string тоже инжектится.

## [7.9] — 2026-09-13

### Fixed
- **RPG Maker: `Failed to load audio/bgm/...` после перевода** — имена аудио/картинок/видео без расширения (напр. `001 Test Song`) извлекались как текст и переводились, игра просила у диска несуществующий файл. Новый `app/core/rpgmaker/resrefs.py` строит индекс ресурсов игры (audio/img/movies, шифрованные `.ogg_`/.rpgmvp, www-деплой): совпавшие строки — это ссылки на файлы и не переводятся никогда. Фильтр стоит в извлечении (события, параметры, литералы, generic), в `engine.apply` (чистит и старые проекты, счётчик `res_skipped`), в `parser.apply`/`_replace_js_strings`/`_replace_param_value` и в JS-обходе памяти (`obIsAudio` + `obIsResKey`: аудио-объекты и `characterName`/`faceName`/`battlerName`/фоны/параллакс не подменяются).
- **RPG Maker: нетекстовые команды событий больше не извлекаются** — `CMD_NONTEXT_CODES` (метки 118/119, переменные со скриптом 122, BGM/ME 132/133/139/140, маршрут 205/505, переход 222, картинки 231–235, аудио 241–250, видео 251, фоны 283/284, изображения 322/323, ветвление 111): там только файлы/метки/код, отображаемого текста нет.
- **RPG Maker: verify ловит перевод имени файла** — `verify_resource_refs` сравнивает backup и текущий `plugins.js` полистно: значение-ресурс, заменённое на нефайл, даёт понятную ошибку с именем параметра и лечится откатом (в сообщении — подсказка про skip).

## [7.8] — 2026-09-13

### Fixed
- **RPG Maker: `SyntaxError Expected ',' or ']' after array element in JSON` после перевода** — корень: извлечение спускалось внутрь JSON-в-строке параметров плагинов (базы типа AnotherNewGame), а запись делала слепую подстроку во внешней строке. Кавычка/бэкслеш в переводе рвали внутреннюю структуру, внешний `plugins.js` оставался валидным, а плагин падал на своём `JSON.parse` при старте игры. Теперь замена в параметрах JSON-осознанная (`parser.py:_replace_param_value`): decode → точный лист по индексу `:n` → encode → проверка `json.loads`; не сошлось — строка пропускается, файл не трогается.
- **RPG Maker: структура перевода с гарантией запуска** — новый `app/core/rpgmaker/verify.py`: после каждого apply все файлы, которые движок читает через `JSON.parse` (`data/*.json`, `.rpgmvm`, списки плагинов обоих форматов, вшитые словари), проверяются строгим парсером глазами V8 (NaN/Infinity, висячие запятые и комментарии отвергаются). При провале — автоматический откат всего нашего (`uninstall_runtime` + `restore_original`) и честная ошибка в UI (`tr_verify_failed`/`tr_verify_restored`/`tr_verify_broken`) вместо молчаливого «готово». Пропущенная строка лучше мёртвой игры.

## [7.7] — 2026-09-13

### Fixed
- **RPG Maker: перевод больше не ломает запуск игры** — `js/plugins/*.js` по умолчанию НЕ патчатся файлом (замена литералов по содержимому рвала синтаксис и логику: один литерал — и текст, и ключ). Тексты плагинов идут в runtime-словарь и переводятся live через catch-all хуки (`drawText`/`drawTextEx`/`convertEscapeCharacters`) без touching файлов. Рискованный file-patch доступен только явно (`patch_plugin_js=True`). `data/*.json`/`Map*.json` как и раньше — только `ob_translation/<lang>.json` + `ob_runtime.js`, оригиналы байт-в-байт целы. Для asar — без репака архива, только overlay + live.
- **RPG Maker: ES5-пейлоады для старого NW.js (MV)** — `PAYLOAD` (читы), `TRANSPORT_SHIM` (CDP-канал) и все чит-выражения переписаны без `const`/`let`/`=>`/`includes` (`var`/`function`/`indexOf`). Старый Chromium 41-49 ронял игру с `SyntaxError` на старте/инъекции, читы «молча не работали». Пейлоады вынесены в `app/core/rpgmaker/payloads.py` (ядро без Qt), `tentacle.py` их реэкспортирует.
- **RPG Maker: `</script>`/`<!--` в переводах больше не дают пустой экран** — `js_json` (`mv_bridge`), `_js_escape_translation` (`parser`) и вывод `plugins.js` уводят `<`/`>`/`&` в `\uXXXX` (в рантайме декодируются обратно). Плагины грузятся через `<script src>` — `</script>` внутри строки закрывал элемент на уровне HTML-парсера раньше JS-парсера.
- **RPG Maker: `_replace_js_strings` не трогает код** — при apply повторно проверяется `_is_code_literal`: одинаковый литерал как текст и как ключ (`command==='...'`, `args["..."]`) — кодовое вхождение пропускается, тихий софт-брейк исключён.
- **RPG Maker: `plugins.js` с валидацией и бэкапом** — запись `ob_runtime`/`octopus_ob` и удаление проверяются парсингом (JSON/JS-массив) перед записью; оригинал — в `<plugins.js>.ob_backup`; удаление — строко-чувствительное (скобки внутри переводов не считаются). Битый `plugins.js` = игра не стартует — теперь не пишется.
- **RPG Maker: читы работают как нужно** — `gold_set` через `gainGold(diff)` (хуки + UI), `teleport` блокируется в бою, `reload_map` без `Decrypter` (сигнатура `R` + `www/data`-фолбэк + fallback на `setup`), `open_*` не дублируют сцену, `actor_set.exp` — числом, все IIFE — ES5. `request_state` и хуки — с `try/catch`, `collectState` — с защитой от несозданных `$game*`.
...
### Added
- **RPG Maker MV/MZ: runtime-overlay translation ("variant 3")** — `apply()` no longer modifies game files at all. Translations are written to an isolated JSON overlay `ob_translation/<lang>.json` next to the game (`www/ob_translation/` for www-deploys), and a generated `ob_runtime.js` plugin (`app/core/rpgmaker/runtime.py`: `install_runtime`/`uninstall_runtime`, registered in the plugins list of either format) embeds the dictionary and translates in memory. Original `data/*.json` stay byte-identical, so games with custom plugins or unusual database schemas can no longer be corrupted by patching; rollback = remove the plugin file, the overlays and the plugins.js entry. Projects patched by the old file-based method are migrated once on apply (existing `backup/` is restored automatically). Electron/asar builds are never repacked — disk overlay only, live translation over CDP when launched through OctopusBridge (`asar_live_only` stats flag).
- **RPG Maker: data-level runtime translation** — menus, items, skills and system terms are drawn straight from `$data*` tables without passing through dialog hooks, which made the first runtime build look untranslated. The shared translation payload (live bridge + `ob_runtime.js`) now walks the loaded database in memory (`__octopus_trWalk`): all core tables plus `$dataMap`, exact-match string replacement only, `note`/`meta` keys skipped. Event commands are translated positionally via a whitelist of player-visible parameters — speaker name 101[4], choices 102[0], rename/nickname 320/324[1], MV plugin command 356[0], MZ plugin args 357[3], dialog lines 401[0] / scroll text 405[0], choice branches 402[1]. Comments 108/408 and script literals 355/655 are deliberately not touched (plugins read them as tags/code). `$dataMap` is re-walked after every `Game_Map.setup`, and a late dictionary push (live mode) triggers an immediate re-walk once the database exists.
- **RPG Maker: extraction of non-CJK text from unknown sources** — games mixing Japanese and English lost the English parts when they lived in custom structures. The generic fallback for unknown event-command codes, unknown JSON files and custom DB fields now accepts natural-language Latin strings (whitespace-bearing phrases filtered through `js_text_candidate`: paths/URLs/single tokens rejected) alongside CJK, and recurses into nested parameter structures up to depth 4 (`_Extractor._generic_walk_value`). Safe under variant 3: false positives can no longer corrupt files since application happens in game memory only.

### Changed
- **Mixed-language games translate fully regardless of the source setting** (`app/core/translate/service.py`): source detection is per-line even when the project source language is explicit (`_resolve_src`). Lines detected in another language are routed to the engine under their real language — English UI text inside a "Japanese" project is translated instead of being sent to the engine labeled as Japanese; lines already in the target language are skipped in explicit-source mode too (previously they were sent pointlessly).

### Fixed
- **MV bridge: broken JS when a translation contains `");"`** — the static dictionary was injected into `octopus_ob.js` via a non-greedy regex that truncated at the first `);` inside translated text, producing a syntax error that killed the bridge plugin entirely (reported by a user on their own game). Injection/replacement now uses a string-aware balanced scanner respecting quotes and nesting (`mv_bridge._tr_dict_span`); injected dictionaries are escaped for U+2028/U+2029 line separators (`js_json`, reused by tentacle payloads); a corrupted plugin self-heals on the next ensure pass by regenerating from the current project dictionary.
- **MZ plugins list restore**: removing the runtime entry from `data/plugins.js` (JSON array) parses and filters via json instead of the MV-oriented text scanner, leaving no trailing-comma artifacts behind.
